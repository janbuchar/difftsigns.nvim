...
describe("verdict.compute", function()
  ...
  describe("verdict.at_line", function()
    it("finds the hunk covering a buffer line", function()
      local set = verdict.compute({ hunk("change", 2, 5, 2, 3) }, fixture("wrap_in_if"))
      assert.are.equal(set.verdicts[1], verdict.at_line(set, 4))
      assert.is_nil(verdict.at_line(set, 40))
    end)

    it("finds a delete hunk by its anchor, since it covers no lines", function()
      local set = verdict.compute({ hunk("delete", 4, 0, 5, 3) }, fixture("deletion"))
      assert.are.equal(set.verdicts[1], verdict.at_line(set, 4))
      assert.are.equal(set.verdicts[1], verdict.at_line(set, 5))
    end)
  end)
end)
